...
func GeneratePRContentWithClaude(config *Config, diff string) (title, body string, err error) {
	...
	combinedPrompt := buildCombinedPrompt(config, filteredDiff)
...
func buildCombinedPrompt(config *Config, diff string) string {
